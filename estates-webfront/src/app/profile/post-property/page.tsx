...
import { uploadToR2 } from "@/lib/agentAuth";
...
export default function PostPropertyPage() {
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      try {
        const url = await uploadToR2(files[i], "properties", getToken());
        urls.push(url);
  ...
  async function handleBrochureUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const url = await uploadToR2(file, "documents", getToken());
      set("brochureUrl", url);
    } catch {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await authFetch("/api/properties", { method: "POST", body: JSON.stringify(payload) });
      if (!res.ok) {
        const d = await res.json();
        toast.error(d.error ?? d.message ?? "Submission failed. Please try again.");
        setSubmitting(false);
        return;
      }
  ...
  return (
    ...
    >
      <div>
        <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1">List Your Property</p>
        <h1 className="text-2xl font-bold text-neutral-900 tracking-tight">Post Property</h1>
        <p className="text-sm text-neutral-500 mt-1">Submit your property for review — it goes live once approved by our team.</p>
      </div>
